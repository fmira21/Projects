...
*/
	...
	try
	{
		...
		Form* scf;
		scf = int0.makeForm("shrubbery creation", "toShrub");
		std::cout << BOLDWHITE << *scf << DEFAULT;
		std::cout << std::endl;

		//RobotomyRequestForm rrf("toRobo");
		Form* rrf;
		rrf = int0.makeForm("robotomy request", "toRobo");
		std::cout << BOLDWHITE << *rrf << DEFAULT;
		std::cout << std::endl;

		//PresidentialPardonForm ppf("toPardon");
		Form* ppf;
		ppf = int0.makeForm("presidential pardon", "toPardon");
		std::cout << BOLDWHITE << *ppf << DEFAULT;
		std::cout << std::endl;

		/* Invalid case with a random name:
		Form* random;
		random = int0.makeForm("abracadabra", "random");
		*/
