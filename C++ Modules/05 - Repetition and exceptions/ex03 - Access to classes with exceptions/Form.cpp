...
{
    return ("Error: This form is already executed");
}
...
: _name(name), _gradeToSign(gradeToSign), _gradeToExecute(gradeToExecute), _isSigned(false), _isExecuted(false)
{
    checkFormGrades(_gradeToSign, _gradeToExecute);
// NB: Here we initialize the unsigned and unexecuted form by default.
}
...
void Form::execute (Bureaucrat const & executor) const
{
    if (_isSigned == false)
    {
        std::cout << BOLDRED << "Bureaucrat " << executor.getName() << " cannot execute this form because: ";
        throw Form::notSignedException();
    }
    if (_isExecuted == true)
    {
        std::cout << BOLDRED << "Bureaucrat " << executor.getName() << " cannot execute this form because: ";
        throw Form::alreadyExecutedException();
    }
